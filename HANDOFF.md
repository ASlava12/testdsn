...
- The root `REPOSITORY_STAGE` marker and `overlay_core::REPOSITORY_STAGE` both
  read `milestone-19-pilot-closure`.
...
- Separate-host evidence is still required before claiming regular distributed
  pilot closure.
...
- off-box evidence is still required on separate hosts
