...
- The root `REPOSITORY_STAGE` marker and `overlay_core::REPOSITORY_STAGE` both
  read `milestone-21-first-user-runtime`.
...
- Milestone 21 first-user runtime is the current stage with bounded recovery of
...
- Separate-host evidence is still required on the validated commit before
  claiming first-user runtime readiness for that release candidate.
...
## Remaining blockers for first-user runtime
...
- off-box evidence is still required on separate hosts for the exact release
  commit before claiming first-user runtime readiness

## Recommended first Codex task

Use `prompts/codex-milestone-21.md` as the first task prompt for the current
`milestone-21-first-user-runtime` stage. It assumes the repository already has a
...
1. Confirm from `README.md`, `AGENTS.md`, and `IMPLEMENT.md` that the current
   stage is `milestone-21-first-user-runtime`.
